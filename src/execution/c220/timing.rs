...
impl C220TimedMte2Core {
    ...
    pub(super) fn core_mut(&mut self) -> &mut MteCoreStepper {
        &mut self.core
    }

    pub(super) fn gate_other_at(
        &mut self,
        tick: u64,
    ...
    pub(super) fn finish_other_at(&mut self, tick: u64) -> Result<(), C220TimingError> {
    ...
    pub fn step_at(
        ...
        let step = self.core.step_mte_word(word, source)?;
        match &step.action {
            MteAction::Issue {
                source_address,
                destination_address,
