...
impl C220Load2dTransfer {
    pub fn segments(self) -> impl ExactSizeIterator<Item = C220Load2dSegment> {
        (0..self.descriptor.repeat_count).map(move |repeat_index| {
            let start_bytes =
                u32::from(self.descriptor.start_index).wrapping_mul(C220_LOAD_2D_BLOCK_BYTES);
            let stride_bytes = u32::from(self.descriptor.source_stride_blocks)
                .wrapping_mul(C220_LOAD_2D_BLOCK_BYTES);
            let repeat_bytes = u32::from(repeat_index).wrapping_mul(stride_bytes);
            let source_offset = match self.descriptor.address_mode {
            ...
            let destination_offset = u64::from(repeat_index)
                .wrapping_mul(u64::from(self.descriptor.destination_stride_blocks()))
