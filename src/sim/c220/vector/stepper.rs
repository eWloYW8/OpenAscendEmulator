...
use crate::isa::c220::vector::{
    C220MovevInstruction, C220ShiftInstruction, C220VecArithmeticHint, C220VecArithmeticOperation,
};
    ...
    pub(crate) fn preview_c220_vector_scalar_word(
        ...
        let iteration_masks = decode_c220_repeat_masks(
            ...
            machine
                .spr_value(101)
                .ok_or(C220VectorError::MissingMaskState)?,
            C220_VECTOR_TILE_BYTES / usize::from(instruction.dtype.element_bytes()),
            control.encoded_repeat_count,
        )?;
        let issue = plan_c220_vector_scalar_issue(
            ...
            C220VectorScalarOperand {
                bits: xregs[usize::from(instruction.scalar_register)] as u32,
                fp16_mode: C220Fp16Mode::from_control_spr(mask_control),
                integer_saturating: mask_control & (1 << 53) != 0,
            },
            control,
            C220VectorAddresses {
    ...
    pub(crate) fn preview_c220_shift_word(
        ...
        let iteration_masks = decode_c220_repeat_masks(
            ...
            machine
                .spr_value(101)
                .ok_or(C220VectorError::MissingMaskState)?,
            C220_VECTOR_TILE_BYTES / usize::from(instruction.element_bytes),
            control.encoded_repeat_count,
        )?;
        Ok(plan_c220_shift_issue(
            pc,
            word,
            xregs[usize::from(instruction.shift_register)] as u32,
            control,
            C220VectorAddresses {
                source_0: xregs[usize::from(instruction.source_register)],
                source_1: 0,
