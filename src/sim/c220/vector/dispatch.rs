...
impl VectorEngine {
    pub(in crate::sim::c220) fn step_at(
        &mut self,
        tick: u64,
        word: u32,
        state: &mut C220State,
    ) -> Result<VectorStep, C220VectorRuntimeError> {
        let pc = state.scalar().pc();
        ...
        let instruction = match word {
            ...
            _ if C220LoadVaInstruction::decode(word).is_some() => {
                ...
                let source_address =
                    state.scalar().machine().xregs()[usize::from(decoded.source_register)];
            ...
            _ if C220NoEffectVectorInstruction::decode(word).is_some() => {
                ...
                let machine = state.scalar().machine();
            ...
            _ if C220PackedCompareInstruction::decode(word).is_some() => {
                ...
                let registers = state.scalar().machine().xregs();
