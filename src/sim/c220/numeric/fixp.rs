...
use super::fp16::{C220Fp16Rounding, c220_f32_to_fp16};
...
mod tests {
    ...
    fn c220_f32_to_fp16(bits: u32, rounding: C220Fp16Rounding) -> C220Fp16Outcome {
        super::c220_f32_to_fp16(bits, rounding, C220Fp16Mode::Saturating)
    }

    #[test]
    fn rounding_modes_cover_ties_tiny_values_and_saturation_status() {
        use C220Fp16Rounding::*;
        ...
        for (bits, expected) in [
            ...
            for (mode, expected) in modes.into_iter().zip(expected) {
                let value = c220_f32_to_fp16(bits, mode);
                assert_eq!(value.bits, expected);
                assert_eq!(value.status.underflow, bits < (103 << 23) && expected == 0);
            }
        }
        for mode in modes {
            assert_eq!(c220_f32_to_fp16(0x8000_0000, mode).bits, 0x8000);
            assert_eq!(c220_f32_to_fp16(65504_f32.to_bits(), mode).bits, 0x7bff);
            assert!(!c220_f32_to_fp16(65504_f32.to_bits(), mode).status.overflow);
            assert!(c220_f32_to_fp16(65536_f32.to_bits(), mode).status.overflow);
            let nan = c220_f32_to_fp16(0xff80_0001, mode);
            assert_eq!(nan.bits, 0);
            assert!(nan.status.nan_operand);
            let inf = c220_f32_to_fp16(f32::NEG_INFINITY.to_bits(), mode);
