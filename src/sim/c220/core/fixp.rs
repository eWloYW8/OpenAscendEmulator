...
impl C220Core {
    ...
    pub fn fixp_sync_bindings(&self) -> Option<&C220FixpSyncBindings> {
        self.fixp.as_ref().map(|fixp| &fixp.bindings)
    }

    pub fn fixp_factors(&self) -> Option<&C220LocalBuffer> {
        self.fixp.as_ref().map(|fixp| &fixp.factors)
    }

    pub fn fixp_factors_mut(&mut self) -> Option<&mut C220LocalBuffer> {
        self.fixp.as_mut().map(|fixp| &mut fixp.factors)
    }
    ...
    pub(super) fn step_fixp_at(
        ...
        let reservation = u64::from(command.descriptor.columns()).div_ceil(16)
