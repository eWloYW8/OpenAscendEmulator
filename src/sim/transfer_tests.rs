...
use crate::sim::c220::mte::mte2::copy_c220_mov_out_to_ub;
use crate::sim::c220::mte::mte3::copy_c220_mov_ub_to_hbm;
...
fn c220_captured_output_route_commits_four_segments_atomically() {
    ...
    let mut destination = output_regions();
    let result = copy_c220_mov_ub_to_hbm(&ub, &mut destination, descriptor, 0x100, 0x3000).unwrap();
...
fn c220_gapped_mov_preserves_skipped_regions_and_rejects_partial_writeback() {
    ...
    let input = C220MovOutToUbDescriptor::decode(CAPTURED_C220_MOV_OUT_TO_UB_X_WORD, xm).unwrap();
